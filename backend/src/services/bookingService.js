...
async function findOrCreatePublicCustomer(customerData) {
  ...
  if (email) {
    ...
    if (existingByEmail) {
      // Never overwrite name — the customer's identity is stable.
      // Only update mutable contact/address fields.
  ...
  if (phone) {
    ...
    if (existingByPhone) {
      // Never overwrite name or email when matching by phone only.
      // A different person can call from the same number.
      // Only update address/license fields.
      return prisma.customer.update({
...
function createBookingDeletionToken(bookingId, expiresIn = "2h") {
  return jwt.sign(
    { purpose: "booking_delete", bookingId: Number(bookingId) },
    getJwtSecret(),
    { expiresIn }
  );
}

function verifyBookingDeletionToken(token) {
  try {
    const payload = jwt.verify(token, getJwtSecret());
    if (payload.purpose !== "booking_delete") {
      throw buildAppError("Invalid deletion token", 403);
    }
    return payload;
  } catch (err) {
    if (err.statusCode) throw err;
    throw buildAppError("Deletion token is invalid or expired", 403);
  }
}

async function deleteBookingByDeletionToken(bookingId, token) {
  const payload = verifyBookingDeletionToken(token);

  if (Number(payload.bookingId) !== Number(bookingId)) {
    throw buildAppError("Deletion token does not match this booking", 403);
  }
  ...
  if (!["reserved"].includes(booking.status)) {
    throw buildAppError("Only reserved bookings can be deleted", 400);
  }

  await prisma.booking.delete({ where: { id: Number(bookingId) } });
  return { deleted: true, bookingId: Number(bookingId) };
}
...
async function createPublicReservation(data) {
  ...
  const deletionToken = createBookingDeletionToken(booking.id);

  let manageToken = null;
  try {
    manageToken = createGuestManageTokenFromBooking(booking);
  } catch {
    // customer lacks email or lastName — manage token not available
  }
...
async function checkoutBooking(id, data, photos = []) {
  ...
  let damageMarkersJsonOut = null;
  if (data.damageMarkersJson) {
    try {
      const parsed = JSON.parse(data.damageMarkersJson);
      damageMarkersJsonOut = JSON.stringify(Array.isArray(parsed) ? parsed : []);
    } catch {
      damageMarkersJsonOut = null;
    }
  }
  ...
  if (damageMarkersJsonOut !== null) {
    await prisma.$executeRaw`UPDATE "Checkout" SET "damageMarkersJson" = ${damageMarkersJsonOut} WHERE "id" = ${checkout.id}`;
  }
...
async function checkinBooking(id, data, photos = []) {
  ...
  let damageMarkersJsonIn = null;
  if (data.damageMarkersJson) {
    try {
      const parsed = JSON.parse(data.damageMarkersJson);
      damageMarkersJsonIn = JSON.stringify(Array.isArray(parsed) ? parsed : []);
    } catch {
      damageMarkersJsonIn = null;
    }
  }
  ...
  if (damageMarkersJsonIn !== null) {
    await prisma.$executeRaw`UPDATE "Checkin" SET "damageMarkersJson" = ${damageMarkersJsonIn} WHERE "id" = ${checkin.id}`;
  }
...
async function updateCheckout(id, data) {
  const booking = await prisma.booking.findUnique({
    where: { id: Number(id) },
    include: { checkout: true },
  });
  if (!booking) throw Object.assign(new Error("Booking not found"), { statusCode: 404 });
  if (!booking.checkout) throw Object.assign(new Error("No checkout record exists for this booking"), { statusCode: 400 });

  const updateData = {};
  if (data.mileageOut !== undefined) updateData.mileageOut = Number(data.mileageOut);
  if (data.fuelLevelOut !== undefined) updateData.fuelLevelOut = data.fuelLevelOut;
  if (data.notesOut !== undefined) updateData.notesOut = data.notesOut || null;
  ...
  if (data.damageMarkersJson !== undefined) {
    let val = null;
    try {
      const parsed = JSON.parse(data.damageMarkersJson);
      val = JSON.stringify(Array.isArray(parsed) ? parsed : []);
    } catch { val = null; }
    await prisma.$executeRaw`UPDATE "Checkout" SET "damageMarkersJson" = ${val} WHERE "id" = ${booking.checkout.id}`;
  }
...
async function updateCheckin(id, data) {
  ...
  if (!booking) throw Object.assign(new Error("Booking not found"), { statusCode: 404 });
  if (!booking.checkin) throw Object.assign(new Error("No checkin record exists for this booking"), { statusCode: 400 });

  const updateData = {};
  if (data.mileageIn !== undefined) updateData.mileageIn = Number(data.mileageIn);
  if (data.fuelLevelIn !== undefined) updateData.fuelLevelIn = data.fuelLevelIn;
  if (data.notesIn !== undefined) updateData.notesIn = data.notesIn || null;
  if (data.damageFee !== undefined) updateData.damageFee = Number(data.damageFee);
  if (data.lateFee !== undefined) updateData.lateFee = Number(data.lateFee);
  if (data.cleaningFee !== undefined) updateData.cleaningFee = Number(data.cleaningFee);
  ...
  if (data.damageMarkersJson !== undefined) {
    ...
    await prisma.$executeRaw`UPDATE "Checkin" SET "damageMarkersJson" = ${val} WHERE "id" = ${booking.checkin.id}`;
  }
